...
class Loader extends PluginBase implements Listener {
    ...
    public function onEnable(): void {
        ...
        $this->getServer()
            ...
            ->register(
                'joinstats',
                new JoinStatsCommand('joinstats', 'See stats', null, ['js'])
            );
        $this->getServer()
            ->getPluginManager()
        ...
        if ($this->provider === null) {
            ...
            if ($provider === null) {
                ...
                ProviderManager::get($provider) === null ||
                $this->getConfig()->get('provider-file', false) === false
            ) {
                $this->getLogger()->critical('Config data is incorrect!');
                $this->setup();
            } else {
                $this->setProvider(
                    ProviderManager::get($provider),
                    $this->getConfig()->get('provider-file')
                );
            }
        }

        if (
            $this->getConfig()->exists('timezone') &&
            $this->getConfig()->get('timezone') !== false
        ) {
            ...
            $this->getLogger()->critical(
                'Timezone value is not set from config. If you don\'t set this up, the wrong times and dates are required.'
            );
            $this->getLogger()->info(
                TextFormat::GRAY .
                    'Timezone is set to "' .
