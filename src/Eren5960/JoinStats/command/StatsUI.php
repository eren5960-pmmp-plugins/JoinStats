...
class StatsUI {
    public static function main(Player $player): void {
        ...
        $form = new SimpleForm(function (Player $player, ?int $index) use (
            $data
        ): void {
            if ($index !== null) {
                self::months(
    ...
    public static function months(
        ...
        $form = new SimpleForm(function (Player $player, ?int $index) use (
            ...
            if ($index !== null) {
                self::days(
                    $player,
                    $months + ['total' => $total],
                    array_values($months)[$index],
                    $year,
                    date(
                        'F',
                        mktime(0, 0, 0, (int) array_keys($months)[$index], 0)
                    )
                );
            }
        });
        ...
        $form->setContent(
            TextFormat::GRAY .
                'Number of players logged into the server this year: ' .
                TextFormat::AQUA .
                $total
        );
    ...
    public static function days(
        ...
        $form = new SimpleForm(function (Player $player, ?int $index) use (
            ...
            if ($index === null || ($index !== 0 && $index % 7 === 0)) {
                ...
                self::hours(
                    ...
                    $months,
                    $days + ['total' => $total],
                    array_values($days)[$index],
                    $year,
                    $month,
                    array_keys($days)[$index]
                );
            }
        });
        ...
        $form->setContent(
            TextFormat::GRAY .
                'Number of players logged into the server this month: ' .
                TextFormat::AQUA .
                $total
        );
    ...
    public static function hours(
        ...
        $form = new CustomForm(function (Player $player) use (
            $months,
            $days,
            $year,
            $month
